...
def _construir_nota_cop(cop_excluidos):
    """
    Construye el texto de la nota 4 sobre COP 62 y 67.
    Retorna None si no hay montos que reportar.
    """
    ...
    if cop62.get('monto', 0) > 0:
        ...
        urs_62 = cop62.get('urs', [])
        urs_str = ', '.join(str(u) for u in urs_62) if urs_62 else '120'
        partes.append(
            f"COP 62 la cantidad de ${monto62:,.2f} ({texto62}) "
            f"esto en la UR {urs_str}"
        )

    if cop67.get('monto', 0) > 0:
        monto67 = cop67['monto']
        texto67 = cop67.get('texto', '') or numero_a_letras_mx(monto67)
        urs_67 = cop67.get('urs', [])
        urs_str = ' y '.join(str(u) for u in urs_67) if urs_67 else '512 y 513'
        partes.append(
            f"COP 67 la cantidad de ${monto67:,.2f} ({texto67}) "
            f"esto en las UR {urs_str}"
        )
...
def generar_excel_sicop(resultados):
    ...
    # Nota 4: COP 62 y 67
    nota4_texto = _construir_nota_cop(cop_excluidos)
    if nota4_texto:
        fila = _nota(fila, nota4_texto, altura=50)
