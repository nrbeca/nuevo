...
def generar_excel_sicop(resultados, df_original=None):
    """
    Genera el archivo Excel de SICOP con formato institucional.
    Notas idénticas a las de la app:
      1/ Exclusión cap 1000 y partida 39801
      2/ Modificado Anual sin congelados
      3/ Modificado Periodo sin congelados
      4/ UR 233 eliminada RI pero reporta recursos (FIJA)
      5/ COP 62/67 sin considerar (dinámica, solo si hay monto)
    """
    ...
    fill_subtotal = PatternFill(start_color='002F2A', end_color='002F2A', fill_type='solid')
    ...
    try:
        ...
        width_px   = int(7.33 * 37.8)
        height_px  = int(1.39 * 37.8)
    ...
    # ── Helpers para escribir filas ───────────────────────────────────────────
    def escribir_fila_subtotal(fila, texto, datos, es_total=False):
        fill_row   = fill_total    if es_total else fill_subtotal
    ...
    def escribir_fila_ur(fila, ur, datos, es_gris=False):
        ...
        denom_map = config['denominaciones']
        ...
        cell_denom           = ws.cell(row=fila, column=2, value=denom_map.get(ur, ''))
    ...
    secciones = [
        ...
        ('oficinas',               'Oficinas de Representación en las Entidades Federativas'),
        ('organos_desconcentrados','Órganos Desconcentrados'),
        ('entidades_paraestatales','Entidades Paraestatales'),
    ]
    for seccion_key, seccion_nombre in secciones:
        escribir_fila_subtotal(fila, seccion_nombre, subtotales[seccion_key])
        fila += 1
        contador = 0
        for ur in config[seccion_key]:
            datos_ur = resumen[resumen['UR'] == ur]
            if not datos_ur.empty:
                escribir_fila_ur(fila, ur, datos_ur.iloc[0].to_dict(), es_gris=(contador % 2 == 1))
                fila += 1
                contador += 1
    ...
    # Nota de fuente
    fila = _nota(fila,
        f'Fuente: Elaborado con la base extraída del Sistema de Contabilidad y '
        f'Presupuesto (SICOP), con corte al {formatear_fecha(ultimo_habil)}.')

    # Nota 1/
    fila = _nota(fila,
        '1/ No Incluye el capítulo 1000 "Servicios Personales" ni partida 39801 '
        '"Impuesto Sobre Nóminas".')

    # Nota 2/
    fila = _nota(fila,
        f'2/ El Presupuesto Modificado Anual no incluye '
        f'${congelados["anual"]:,.2f} ({congelados["texto_anual"]}), recursos congelados.')

    # Nota 3/
    fila = _nota(fila,
        f'3/ El Presupuesto Modificado al periodo no incluye '
        f'${congelados["periodo"]:,.2f} ({congelados["texto_periodo"]}), recursos congelados.')

    # Nota 4/ — UR 233 (FIJA, siempre se incluye)
    fila = _nota(fila,
        '4/ La Unidad Responsable 233 (Dirección General de Agregación de Valor y '
        'Comercialización) fue eliminada de acuerdo con el Reglamento Interior de la '
        'Secretaría de Agricultura y Desarrollo Rural de fecha 31 de diciembre de 2025. '
        'Sin embargo la UR está reportando recursos al periodo.',
        altura=50)

    # Nota 5/ — COP 62/67 (dinámica, solo si hay montos)
    # Recalcular desde df_original usando MODIFICADO_AUTORIZADO (igual que la app)
    import pandas as _pd
    _monto62, _urs62, _monto67, _urs67 = 0, [], 0, []
    _df_orig = df_original if df_original is not None else resultados.get('df_original', None)
    if _df_orig is not None and not _df_orig.empty and 'CONTROL_OPERATIVO' in _df_orig.columns:
        _df_tmp = _df_orig.copy()
        _df_tmp['_COP'] = _pd.to_numeric(_df_tmp['CONTROL_OPERATIVO'], errors='coerce').fillna(-1).astype(int)
        _df_tmp['MODIFICADO_AUTORIZADO'] = _pd.to_numeric(_df_tmp.get('MODIFICADO_AUTORIZADO', 0), errors='coerce').fillna(0)
        ...
        # Fallback al pickle
        _cop_exc = resultados.get('cop_excluidos', {})
        _monto62 = _cop_exc.get('cop_62', {}).get('monto', 0)
        _urs62   = _cop_exc.get('cop_62', {}).get('urs', [])
        _monto67 = _cop_exc.get('cop_67', {}).get('monto', 0)
        _urs67   = _cop_exc.get('cop_67', {}).get('urs', [])
    monto62, urs62_list, monto67, urs67_list = _monto62, _urs62, _monto67, _urs67

    partes_cop = []
    if monto62 > 0:
        urs62_str = ', '.join(str(u) for u in urs62_list) if urs62_list else 'N/D'
        partes_cop.append(
            f"COP 62 la cantidad de ${monto62:,.2f} ({numero_a_letras_mx(monto62)}) "
            f"esto en la UR {urs62_str}"
        )
    if monto67 > 0:
        urs67_str = ', '.join(str(u) for u in urs67_list) if urs67_list else 'N/D'
        partes_cop.append(
            f"COP 67 la cantidad de ${monto67:,.2f} ({numero_a_letras_mx(monto67)}) "
            f"esto en las UR {urs67_str}"
        )

    if partes_cop:
        nota5_texto = (
            "5/ No se están considerando montos de los Controles Operativos (COP): "
            + "; y en ".join(partes_cop) + "."
        )
        fila = _nota(fila, nota5_texto, altura=50)
