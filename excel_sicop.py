...
def generar_excel_sicop(resultados):
    ...
    cop62 = cop_excluidos.get('cop_62', {})
    cop67 = cop_excluidos.get('cop_67', {})
    monto62 = cop62.get('monto', 0)
    monto67 = cop67.get('monto', 0)

    partes_cop = []
    if monto62 > 0:
        texto62  = cop62.get('texto', '') or numero_a_letras_mx(monto62)
        urs62    = cop62.get('urs', [])
        urs62_str = ', '.join(str(u) for u in urs62) if urs62 else 'N/D'
        partes_cop.append(
            f"COP 62 la cantidad de ${monto62:,.2f} ({texto62}) "
            f"esto en la UR {urs62_str}"
        )
    if monto67 > 0:
        texto67  = cop67.get('texto', '') or numero_a_letras_mx(monto67)
        urs67    = cop67.get('urs', [])
        urs67_str = ', '.join(str(u) for u in urs67) if urs67 else 'N/D'
        partes_cop.append(
            f"COP 67 la cantidad de ${monto67:,.2f} ({texto67}) "
