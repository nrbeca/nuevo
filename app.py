...
def calcular_pasivos_cop_desde_sicop(df_original, ur_codigo, config):
    ...
    Pasivos = registros donde:
    - FF 6 + COP 0 (Fuente Financiamiento 6 y Control Operativo 0)
    - FF 1 + COP 10 (Fuente Financiamiento 1 y Control Operativo 10)
    
    Returns:
        dict con Ejercido (suma de EJERCIDO de los registros que cumplen condiciones)
    """
    if df_original is None or df_original.empty:
        return {'PagoCOP': 0}
    
    # Asegurar que tenemos las columnas necesarias
    required_cols = ['ID_UNIDAD', 'FUENTE_FINANCIAMIENTO', 'CONTROL_OPERATIVO', 'EJERCIDO']
    for col in required_cols:
        if col not in df_original.columns:
            # Intentar con variantes del nombre
            if col == 'FUENTE_FINANCIAMIENTO' and 'FF' in df_original.columns:
                df_original['FUENTE_FINANCIAMIENTO'] = df_original['FF']
            elif col == 'FUENTE_FINANCIAMIENTO' and 'FUENTE_FIN' in df_original.columns:
                df_original['FUENTE_FINANCIAMIENTO'] = df_original['FUENTE_FIN']
            else:
                return {'PagoCOP': 0}
    
    # Mapear UR
    df_original['ID_UNIDAD'] = df_original['ID_UNIDAD'].astype(str)
    
    # Filtrar por UR (considerando mapeos)
    mapeo_ur = config.get('mapeo_ur', {})
    fusion_urs = config.get('fusion_urs', {})
    
    # Crear lista de URs que corresponden a la UR seleccionada
    urs_a_buscar = [ur_codigo]
    ...
    # Filtrar por URs
    df_ur = df_original[df_original['ID_UNIDAD'].isin(urs_a_buscar)].copy()
    
    if df_ur.empty:
        return {'PagoCOP': 0}
    
    # Asegurar tipos numéricos
    df_ur['FUENTE_FINANCIAMIENTO'] = pd.to_numeric(df_ur['FUENTE_FINANCIAMIENTO'], errors='coerce').fillna(0).astype(int)
    ...
    # Filtrar por las condiciones de pasivos:
    # 1) FF=6 y COP=0
    # 2) FF=1 y COP=10
    condicion_pasivos = (
        ((df_ur['FUENTE_FINANCIAMIENTO'] == 6) & (df_ur['CONTROL_OPERATIVO'] == 0)) |
        ((df_ur['FUENTE_FINANCIAMIENTO'] == 1) & (df_ur['CONTROL_OPERATIVO'] == 10))
    )
    
    df_pasivos = df_ur[condicion_pasivos]
    
    # Sumar ejercido
    pago_cop = df_pasivos['EJERCIDO'].sum()
    
    return {'PagoCOP': round(pago_cop, 2)}
...
# ============================================================================
    ...
    with tab2:
        ...
        if ur_rows.empty:
            ...
            with col_izq:
                ...
                # Calcular pasivos pagados en COP desde SICOP
                pasivos_cop = calcular_pasivos_cop_desde_sicop(df_original, ur_codigo, config)
                pago_cop = pasivos_cop.get('PagoCOP', 0)
                ...
                with cp2:
                    valor_cop = format_currency(pago_cop)
                    st.markdown(f'<div style="border:1px solid #ddd;border-radius:8px;padding:1rem;text-align:center;"><div style="font-size:0.8rem;color:#666;">Pasivos pagados en COP</div><div style="font-size:1.2rem;font-weight:bold;color:#002F2A;">{valor_cop}</div></div>', unsafe_allow_html=True)
                
                st.markdown("**Avance de pago de pasivos**")
                
                if pasivos_shcp > 0 and pago_cop > 0:
                    pct_pagado = min(pago_cop / pasivos_shcp, 1)
    ...
    with tab3:
        ...
        # Sección de Pasivos en Austeridad
        st.markdown("#### Pasivos con cargo al presupuesto")
        ...
        with cp1:
            st.markdown(f'<div style="border:1px solid #ddd;border-radius:8px;padding:1rem;text-align:center;"><div style="font-size:0.8rem;color:#666;">Pasivos reportados a la SHCP</div><div style="font-size:1.2rem;font-weight:bold;color:#9B2247;">{format_currency(pasivos_shcp)}</div></div>', unsafe_allow_html=True)
        with cp2:
            st.markdown(f'<div style="border:1px solid #ddd;border-radius:8px;padding:1rem;text-align:center;"><div style="font-size:0.8rem;color:#666;">Pasivos pagados en COP</div><div style="font-size:1.2rem;font-weight:bold;color:#002F2A;">{format_currency(pago_cop)}</div></div>', unsafe_allow_html=True)
        with cp3:
            por_pagar = max(0, pasivos_shcp - pago_cop)
            st.markdown(f'<div style="border:1px solid #ddd;border-radius:8px;padding:1rem;text-align:center;"><div style="font-size:0.8rem;color:#666;">Pasivos por pagar</div><div style="font-size:1.2rem;font-weight:bold;color:#ED7D31;">{format_currency(por_pagar)}</div></div>', unsafe_allow_html=True)
        
        # Gráfica de pasivos
        if pasivos_shcp > 0:
            pct_pagado = min(pago_cop / pasivos_shcp, 1) if pasivos_shcp > 0 else 0
            fig_pas = go.Figure(go.Pie(
                values=[pago_cop, max(0, pasivos_shcp - pago_cop)], 
                labels=['Pagado', 'Por pagar'], 
                hole=0.6, 
                marker_colors=[COLOR_NARANJA, COLOR_AZUL], 
                textinfo='none'
            ))
            fig_pas.add_annotation(text=f"{pct_pagado*100:.1f}%", x=0.5, y=0.5, font_size=20, font_color=COLOR_VINO, showarrow=False)
            fig_pas.update_layout(showlegend=True, legend=dict(orientation="h", y=-0.1), margin=dict(t=10, b=30, l=10, r=10), height=200)
            
            col_graf, col_esp = st.columns([1, 2])
            with col_graf:
                st.plotly_chart(fig_pas, use_container_width=True, key="fig_aust_pasivos")
