     ...
     # ====================================================================
        ...
        # ====================================================================
            ...
            with tab1:
                ...
                # Notas al pie SICOP
                st.markdown("---")
                ultimo_habil_notas = obtener_ultimo_dia_habil(date.today())
                congelados = resultados.get('congelados', {})
                
                st.markdown(f"**Fuente:** Elaborado con la base extraída del Sistema de Contabilidad y Presupuesto (SICOP), con corte al {formatear_fecha(ultimo_habil_notas)}.")
                st.markdown("**Notas:**")
                st.markdown('1/ No Incluye el capítulo 1000 "Servicios Personales" ni partida 39801 "Impuesto Sobre Nóminas".')
                
                cong_anual = congelados.get('anual', 0)
                texto_anual = congelados.get('texto_anual', 'Cero pesos 00/100 M.N.')
                st.markdown(f"2/ El Presupuesto Modificado Anual no incluye \\${cong_anual:,.2f} ({texto_anual}), recursos congelados.")
                
                cong_periodo = congelados.get('periodo', 0)
                texto_periodo = congelados.get('texto_periodo', 'Cero pesos 00/100 M.N.')
                st.markdown(f"3/ El Presupuesto Modificado al periodo no incluye \\${cong_periodo:,.2f} ({texto_periodo}), recursos congelados.")
                
                # Nota 4: COP 62 y 67 - SIEMPRE se muestra
                cop_excluidos = resultados.get('cop_excluidos', {})
                cop62 = cop_excluidos.get('cop_62', {'monto': 0, 'urs': ['120'], 'texto': 'Cero pesos 00/100 M.N.'})
                cop67 = cop_excluidos.get('cop_67', {'monto': 0, 'urs': ['512', '513'], 'texto': 'Cero pesos 00/100 M.N.'})
                
                urs_62 = ', '.join(str(u) for u in cop62.get('urs', ['120']))
                texto_62 = cop62.get('texto', 'Cero pesos 00/100 M.N.')
                urs_67 = ' y '.join(str(u) for u in cop67.get('urs', ['512', '513']))
                texto_67 = cop67.get('texto', 'Cero pesos 00/100 M.N.')
                
                nota_cop = f"4/ No se están considerando montos de los Controles Operativos (COP): COP 62 la cantidad de \\${cop62.get('monto', 0):,.2f} ({texto_62}) esto en la UR {urs_62}; y en COP 67 la cantidad de \\${cop67.get('monto', 0):,.2f} ({texto_67}) esto en las UR {urs_67}."
